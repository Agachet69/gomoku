    ...
    for dir in [1, -1]:
        for length in range(2, 6):
            ...
            if number_player < length:
                if values[-1] == opponent or not (
                    0 <= last_move[0] + length * dir < 19
                ):
                    blocked += 1
                break
        ...
        for down in [1, -1]:
            for length in range(2, 6):
                ...
                if number_player < length:
                    if (
                        values[-1] == opponent
                        or not (0 <= last_move[0] + length * (1 if right else -1) < 19)
                        or not (0 <= last_move[1] + length * down < 19)
                    ):
                        longest[right][down]["blocked"] = 1
                    break
