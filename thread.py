...
def find_longest_diag(board, last_move):
    ...
    for right in [True, False]:
        for down in [1, -1]:
            for length in range(2, 6):
                if right:
                    values = kern_trad(board, get_kern_diag_idx(last_move, slope=[1, down], length=length))
                else:
                    if last_move[0] - length < 0:
                        longest[right][down]["blocked"] = 1
                        break
                    values = kern_trad(flipped_board, get_kern_diag_idx(flipped_move, slope=[1, down], length=length))
                number_player = np.count_nonzero(values == player)
    ...
    size_diag_high_left_down_right = max(longest[True][1]["values"]) + max(longest[False][-1]["values"])
    size_diag_down_left_high_right = max(longest[False][1]["values"]) + max(longest[True][-1]["values"])
    longest_size = max(size_diag_high_left_down_right, size_diag_down_left_high_right) - 1
...
def find_longest_opponent_row(board, last_move: Tuple[int, int]):
    ...
    for dir in [1, -1]:
        for length in range(1, 5):
            values = kern_trad(board, get_kern_row_idx([last_move[0] + dir, last_move[1]], direction=dir, length=length))
...
def find_longest_opponent_col(board, last_move: Tuple[int, int]):
    ...
    for dir in [1, -1]:
        for length in range(1, 5):
            values = kern_trad(board, get_kern_col_idx([last_move[0], last_move[1] + dir], direction=dir, length=length))
            number_player = np.count_nonzero(values == opponent)
...
def find_longest_opponent_diag(board, last_move: Tuple[int, int]):
    ...
    for right in [True, False]:
        for down in [1, -1]:
            for length in range(1, 5):
                if right:
                    values = kern_trad(board, get_kern_diag_idx([last_move[0] + 1, last_move[1] + down], slope=[1, down], length=length))
                else:
                    if last_move[0] - length < 0:
                        break
                    values = kern_trad(flipped_board, get_kern_diag_idx([flipped_move[0] + 1, flipped_move[1] + down], slope=[1, down], length=length))
                number_player = np.count_nonzero(values == opponent)
    ...
    return max(max(longest[True][1]) + max(longest[False][-1]), max(longest[False][1]) + max(longest[True][-1])) - 1
    ...
    for get_idx, (dx, dy) in directions:
        for sign in [1, -1]:
            for offset in [0, -3]:
                ...
                if (np.any(x_idx < 0) or np.any(x_idx >= board_shape[1]) or
                    np.any(y_idx < 0) or np.any(y_idx >= board_shape[0])):
                    continue
