    ...
    for x, y in winning_seq:
        for dx, dy in directions:
            ...
            if (
                0 <= x2 < BOARD_SIZE
                and 0 <= y2 < BOARD_SIZE
                and 0 <= neg_x < BOARD_SIZE
                and 0 <= neg_y < BOARD_SIZE
            ):
                if (
                    board[y1, x1] == player_value
                    and board[y2, x2] == opp_value
                    and board[neg_y, neg_x] == 0
                ):
                    return True
                elif (
                    board[y1, x1] == player_value
                    and board[y2, x2] == 0
                    and board[neg_y, neg_x] == opp_value
                ):
                    return True
            x1, y1 = x - dx, y - dy
            x2, y2 = x1 - dx, y1 - dy
...
def can_capture_winning_sequences(board, winning_sequences, player_value, game):
    ...
    return can_capture_from_winning_sequence(
        board, winning_sequences[0], player_value, game
    )
